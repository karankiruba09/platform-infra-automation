...
- **Activity Log**: Server-side log that updates when new collector data is observed
...
- **`api.py`**: Flask web server that:
  - Server-renders the dashboard UI (HTML + CSS; no JavaScript)
  - Provides REST API endpoint `/api/v1/vcenters` to fetch upgrade data (optional)
  - Securely handles static file serving with path traversal protection

### Frontend
- **`templates/index.html`**: Server-rendered dashboard UI layout (KPIs, status table, activity log)
...
3. Click **Refresh** to reload the page (the collector is run separately)
4. Monitor the activity log for status updates and errors
...
3. Modify the table in `templates/index.html` to display new columns
